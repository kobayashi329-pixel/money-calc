...
**sitemap**: `app/sitemap.ts` はレジストリ駆動で live 計算機を自動収録。`lastModified`（ビルド時刻）付き。全27URL（計算機16＋カテゴリ6＋信頼性4＋トップ）。`app/robots.ts` がsitemap場所を明示。新規ページ公開後は Search Console で URL検査→インデックス登録リクエスト（sitemap再送信は任意・自動再クロールされる。Googleのpingエンドポイントは2023年廃止済みなので手動不要）。
